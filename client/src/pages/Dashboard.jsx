...
import {FilePenLineIcon, Flag, LoaderCircle, LoaderCircleIcon, PencilIcon, PlusIcon, TrashIcon, UploadCloud, UploadCloudIcon, XIcon} from 'lucide-react'
...
const Dashboard = () => {
  ...
  const loadAllResumes = async () =>{
    try {
      const {data} = await api.get('/api/users/resumes', {headers: getAuthHeaders(token)})
  ...
  const createResume = async(event) => {
    try {
      ...
      const {data} = await api.post('/api/resumes/create', {title}, {headers: getAuthHeaders(token)})
  ...
  const uploadResume = async (event) => {
    ...
    try {
      ...
      const {data} = await api.post('/api/ai/upload-resume', {title, resumeText},  {headers: getAuthHeaders(token)})
      setTitle('')
      setResume(null) 
  ...
  const editTitle = async (event) =>{
    try {
      event.preventDefault()
      const {data} = await api.put('/api/resumes/update', {resumeId: editResumeId, resumeData: {title}},  {headers: getAuthHeaders(token)})
  ...
  const deleteResume = async (resumeId) =>{
    try {
      ...
      if(confirm){
        const {data}  = await api.delete(`/api/resumes/delete/${resumeId}`, {headers: getAuthHeaders(token)})
      ...
      px-4 py-8'>
        ...
        <div className='flex gap-4'>
          <button onClick={()=> setShowCreateResume(true)} className='w-full bg-white sm:max-w-36 h-48 flex
           flex-col items-center justify-center rounded-lg gap-2 text-slate-600 border border-dashed
            border-slate-300 group hover:border-indigo-500 hover:shadow-lg translate-all duration-300 cursor-pointer'>
            <PlusIcon className='size-11 translate-all duration-300 p-2.5 bg-gradient-to-br
          ...
          rounded-lg gap-2 text-slate-600 border border-dashed border-slate-300 group hover:border-purple-500 hover:shadow-lg translate-all duration-300 cursor-pointer'>
        ...
        <div className='grid grid-cols-2 sm:flex flex-wrap gap-4'>
          {allResumes.map((resume, index)=>{
            ...
            return (
              <button onClick={()=> navigate(`/app/builder/${resume._id}`)} key={index} className='relative w-full sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-2lg gap-2
              border group hover:shadow-lg transition-all duration-300 cursor-pointer' style={{background: `linear-gradient(135deg, ${baseColor}10, ${baseColor}40)`, borderColor:baseColor + 40 }}>
                <FilePenLineIcon className='size-7 group-hover:scale-105 transition-all' style={{color: baseColor}}/>
                <p className='text-sm group-hover:scale-105 transition-all px-2 text-center' style={{color: baseColor}}>{resume.title}</p>
                <p className='absolute bottom-1 text-[11px]'>Updated on {new Date(resume.updatedAt).toLocaleDateString()}</p>
                <div onClick={e=> e.stopPropagation()} className='absolute top-1 right-1 group-hover:flex items-center hidden'>
                  <TrashIcon onClick={()=>deleteResume(resume._id)} className='size-7 p-1.5 hover:bg-white/50 rounded-2xl text-slate-700 transition-colors'/> 
                  <PencilIcon onClick={()=> {setEditResumeId(resume._id); setTitle(resume.title)}} className='size-7 p-1.5 hover:bg-white/50 rounded-2xl text-slate-700 transition-colors'/>
                </div>
              </button>
            )
          })}
