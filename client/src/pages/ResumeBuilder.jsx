...
import { ArrowLeftIcon, Briefcase, ChevronLeft, ChevronRight, DownloadIcon, EyeIcon, EyeOffIcon, FileText, FolderIcon, GraduationCap, Share2Icon, Sparkles, User, UserIcon } from 'lucide-react'
...
const normalizeResumeData = (resume = {}) => ({
  ...
  project: resume.project || resume.projects || defaultResumeData.project,
})
...
const ResumeBuilder = () => {
  ...
  const loadExistingResume = async () => {
    try {
      const {data} = await api.get('/api/resumes/get/' + resumeId, {headers: getAuthHeaders(token)})
      if(data.resume){
  ...
  const changeResumeVisibility = async()=> {
    try {
      ...
      const {data} = await api.put('/api/resumes/update' ,formData, {headers: getAuthHeaders(token)})
  ...
  const saveResume = async() => {
    try {
      ...
      setResumeData(prev => data.resume ? normalizeResumeData(data.resume) : prev)
