...
const ExperienceForm = ({ data, onChange }) => { // Added props
  ...
  const generateDescription = async (index) =>{
    ...
    try {
      const {data} = await api.post('/api/ai/enhance-job-desc' , {userContent: prompt }, {headers: getAuthHeaders(token)})
            ...
            <div key={index} className='p-4 border border-gray-200 rounded-lg space-y-3'>
              ...
              <div className='space-y-2'>
                <div className='flex items-center justify-between'>
                  ...
                  <button onClick={()=> generateDescription(index)} disabled={generatingIndex === index || !experience.position || !experience.company} className='flex items-center gap-1 text-xs text-purple-600 hover:text-purple-700'>
